  ...
  const fetchCandidates = async () => {
    ...
    try {
      ...
      try {
        ...
        const candidatesResponse = await fetch('/api/candidate/all', { headers });
        if (candidatesResponse.ok) {
          data = await candidatesResponse.json();
          console.log('Raw candidates data:', data);
        } else {
          console.warn('Failed to fetch candidates:', candidatesResponse.status);
        }
