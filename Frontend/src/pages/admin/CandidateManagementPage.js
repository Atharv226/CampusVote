...
const CandidateManagementPage = () => {
  ...
  const isApprovedCandidate = (c) => (c?.isApproved === true) || (c?.status === 'approved');
  ...
  const loadData = async (showLoadingSpinner = true) => {
    ...
    try {
      ...
      // Fetch elections and candidates; support both backend variants
      const els = await axios.get('/api/admin/elections', { headers });
      const cands = await axios.get('/api/admin/candidates', { headers });
      
      // Elections can be an array (preferred) or wrapped
      const electionsData = Array.isArray(els.data) ? els.data : (els.data?.elections || []);
      setElections(electionsData);
      
      // Candidates can be an array (preferred) or wrapped
      const rawCands = cands.data;
      const baseCandidates = Array.isArray(rawCands) ? rawCands : (rawCands?.candidates || []);
      let allCandidates = baseCandidates;
      
      // Optionally merge pending candidates if the endpoint exists (production-server.js style)
      try {
        const pendingResp = await axios.get('/api/admin/candidates/pending', { headers });
        const pendingList = Array.isArray(pendingResp.data) ? pendingResp.data : (pendingResp.data?.candidates || []);
        if (pendingList.length) {
          // Mark as pending and merge (avoid duplicates by _id)
          const pendingMarked = pendingList.map(c => ({ ...c, isPending: true }));
          const seen = new Set(allCandidates.map(c => c._id || c.id));
          allCandidates = [
            ...allCandidates,
            ...pendingMarked.filter(c => !seen.has(c._id || c.id))
          ];
        }
      } catch (pendingErr) {
      ...
      if (!selectedElection && electionsData?.length) {
        setSelectedElection(electionsData[0]._id || electionsData[0].id);
      }
    } catch (e) {
  ...
  useEffect(() => {
    // Initialize filters from query params, e.g. /admin/candidates?view=pending
  ...
  const approve = async (candidateId, candidateName = '', skipConfirmation = false) => {
    ...
    try {
      ...
      // Try new route first; if not available, fall back to status endpoint
      try {
        await axios.post(`/api/admin/candidates/${candidateId}/approve`, {}, { headers });
      } catch (err) {
        if (err.response?.status === 404) {
          await axios.put(`/api/admin/candidates/${candidateId}/status`, { status: 'approved' }, { headers });
        } else {
          throw err;
        }
      }
  ...
  const reject = async (candidateId, candidateName = '') => {
    ...
    try {
      ...
      await axios.put(`/api/admin/candidates/${candidateId}/status`, { status: 'rejected' }, { headers });
  ...
  const selectAllCandidates = () => {
    const pendingCandidates = getFilteredCandidates().filter(c => !isApprovedCandidate(c));
  ...
  const bulkApprove = async () => {
    ...
    try {
      ...
      // Process approvals in parallel with fallback support
  ...
  const getFilteredCandidates = () => {
    ...
    if (statusFilter === 'pending') {
      filtered = filtered.filter((c) => !isApprovedCandidate(c));
  ...
  const pending = filteredCandidates.filter((c) => !isApprovedCandidate(c));
  const approved = filteredCandidates.filter((c) => isApprovedCandidate(c));
  
  // Statistics
  const totalCandidates = candidates.length;
  const totalPending = candidates.filter(c => !isApprovedCandidate(c)).length;
  const totalApproved = candidates.filter(c => isApprovedCandidate(c)).length;
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="flex items-center justify-between mt-4">
              ...
              <div className="flex items-center space-x-3">
                ...
                {/* Select All Button */}
                {getFilteredCandidates().filter(c => !isApprovedCandidate(c)).length > 0 && (
                  <button
