...
const CandidateRegistrationPage = () => {
  const { user, registerAsCandidate, getCandidateProfile, getCandidateRegistrationElections } = useAuth();
  ...
  useEffect(() => {
    const fetchAvailableElections = async () => {
      const elections = await getCandidateRegistrationElections();
      ...
      if (candidateElections.length > 0) {
        setSelectedElectionId(candidateElections[0]._id || candidateElections[0].id);
      }
    };
