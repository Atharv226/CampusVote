...
const VotingHistoryPage = () => {
  ...
  const fetchVotingHistory = async () => {
    try {
      ...
      const response = await fetch(`/api/voting/history?filter=${filter}&sortBy=${sortBy}`, {
        headers: {
          'x-auth-token': token,
      ...
      if (response.ok) {
        ...
        toast.error(errorData.error || 'Failed to fetch voting history');
      }
    } catch (error) {
  ...
  const fetchStatistics = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('/api/voting/statistics', {
        headers: {
          'x-auth-token': token,
          'Content-Type': 'application/json'
        }
