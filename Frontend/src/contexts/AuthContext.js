...
// Configure axios timeout - baseURL not needed due to proxy in package.json
axios.defaults.timeout = 10000; // 10 seconds timeout
axios.defaults.withCredentials = true;

// Use baseURL only if REACT_APP_API_URL is explicitly set (for production)
...
export const AuthProvider = ({ children }) => {
  ...
  // Get available elections for voters (general)
  ...
  // Get elections open for candidate registration (more specific)
  const getCandidateRegistrationElections = async () => {
    try {
      const response = await axios.get('/api/voter/elections/candidate-registration');
      // Expecting an array
      return Array.isArray(response.data) ? response.data : [];
    } catch (error) {
      console.error('Failed to get candidate registration elections:', error);
      return [];
    }
  };
  ...
  // Get pending candidates (for admin approval)
  const getPendingCandidates = async () => {
    try {
      const response = await axios.get('/api/admin/candidates/pending');
      return response.data;
    } catch (error) {
      console.error('Failed to get pending candidates:', error);
      return [];
    }
  };

  // Approve or reject a candidate (admin only)
  const updateCandidateStatus = async (candidateId, status, reason = '') => {
    try {
      const response = await axios.put(`/api/admin/candidates/${candidateId}/status`, {
        status,
        reason
      });
      ...
      const message = error.response?.data?.error || `Failed to ${status} candidate`;
      toast.error(message);
      return { success: false, error: message };
    }
  };

  // Get all candidates (public candidate section)
  const getAllPublicCandidates = async () => {
    try {
  ...
  const value = {
    ...
    getBoothInfo,
    getElections,
    getCandidateRegistrationElections,
    getCandidates,
    castVote,
    getResults,
    ...
    registerAsCandidate,
    getCandidateProfile,
    // Admin candidate management functions
    getAllCandidates,
